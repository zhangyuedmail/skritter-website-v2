...
define([], function() {
    ...
    IntervalQuantifier.prototype.quantify = function(item, score) {
        ...
        var actualInterval = now - itemObject.last;
        var factor = 0.9;
        var pctRight = itemObject.successes / itemObject.reviews;
        var scheduledInterval = itemObject.next - itemObject.last;
        //get the factor
        if (score === 2) {
            factor = 0.9;
        } else if (score === 4) {
            ...
            for (var i in divisions) {
                if (itemObject.interval > divisions[i]) {
                    index = i;
                }
            }
        ...
        if (itemObject.successes === itemObject.reviews && itemObject.reviews < 5) {
            factor *= 1.5;
        }
        //decelerate hard items consistently marked wrong
        if (itemObject.reviews > 8) {
            if (pctRight < 0.5) {
                factor *= Math.pow(pctRight, 0.7);
            }
        }
        //multiple by the factor and randomize the interval
        newInterval = this.randomizeInterval(itemObject.interval * factor);
        //bound the interval
        if (score === 1) {
